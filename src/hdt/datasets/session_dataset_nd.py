...
class ToTensorReward(Transform):
    ...
    def __call__(self, sample):
        return {
            ...
            "poss_rtg_nov": torch.FloatTensor(sample["poss_rtg_nov"]),
            "poss_reward_nov": torch.FloatTensor(sample["poss_reward_nov"]),
            "sess_reward_nov": torch.FloatTensor(sample["sess_reward_nov"]),
...
class DataLoaderUDT(IterableDataset):
    def __init__(self, args, type=None, transforms: Optional[List[Transform]] = None):
        if type is None:
            ...
            raise Exception('dataset has to be either None, val or test')
        replay_buffer = replay_buffer.sort_values(by=["userID", 'sessionID'])
        self.num_session_for_user = np.r_[
            0, replay_buffer.groupby("userID", sort=False)["sessionID"].nunique().cumsum().values]
        self.df = replay_buffer.sort_values(by=["userID", 'sessionID'])
        # print(self.df)
    ...
    def __iter__(self):
        ...
        sessRewards_nov = self.df['user_nov_rtgs'].values
        sessRewards_nov = torch.from_numpy(np.fromiter(sessRewards_nov, dtype=np.float64)).float()

        possRewards_nov = self.df['nov_reward'].values
        possRewards_nov = torch.stack([torch.from_numpy(np.array(i, dtype=np.float64)) for i in possRewards_nov]).float()

        possRtgs_nov = self.df['nov_intra'].values
        possRtgs_nov = torch.stack([torch.from_numpy(np.array(i, dtype=np.float64)) for i in possRtgs_nov]).float()
        ...
        while not finished:
            ...
            for i in range(min_session_length):
                ...
                sess_reward_nov = sessRewards_nov[user_session_start + i]
                poss_reward_nov = possRewards_nov[user_session_start + i]
                poss_rtg_nov = possRtgs_nov[user_session_start + i]
                ...
                samples = {
                    ...
                    'poss_rtg': poss_rtg,
                    'sess_reward_nov': sess_reward_nov,
                    'poss_reward_nov': poss_reward_nov,
                    'poss_rtg_nov': poss_rtg_nov
                }
                if self.transforms is not None:
                    for transform in self.transforms:
                        samples = transform(samples)  # only to tensor
...
class TestDataLoaderUDT(IterableDataset):
    def __init__(self, args, type='test', transforms: Optional[List[Transform]] = None):
        if type is None:
            ...
            # test_session_df_replay_buffer_nov2_users_h, test_session_df_replay_buffer_nov1_users_h
                                                    # HDT数据是用旧的return跑的;interdt用新的重跑的
        else:
            raise Exception('dataset has to be either None, val or test')
        replay_buffer = replay_buffer.sort_values(by=["userID", 'sessionID'])
        self.num_session_for_user = np.r_[0, replay_buffer.groupby("userID", sort=False).size().cumsum().values]
