...
def _ensure_novelty_columns(replay_buffer: DataFrame) -> DataFrame:
    replay_buffer = replay_buffer.copy()
    if "user_nov_rtgs" not in replay_buffer.columns:
        if "user_nov_rtg" in replay_buffer.columns:
            replay_buffer["user_nov_rtgs"] = replay_buffer["user_nov_rtg"]
        elif "nov_sess" in replay_buffer.columns:
            replay_buffer["user_nov_rtgs"] = replay_buffer["nov_sess"]
        else:
            replay_buffer["user_nov_rtgs"] = 0.0
    if "nov_reward" not in replay_buffer.columns:
        if "nov_poss" in replay_buffer.columns:
            replay_buffer["nov_reward"] = replay_buffer["nov_poss"]
        else:
            replay_buffer["nov_reward"] = replay_buffer["itemsID"].apply(lambda values: np.zeros(len(values)))
    if "nov_intra" not in replay_buffer.columns:
        if "nov_sess_rtg" in replay_buffer.columns:
            replay_buffer["nov_intra"] = replay_buffer["nov_sess_rtg"]
        elif "nov_poss" in replay_buffer.columns:
            replay_buffer["nov_intra"] = replay_buffer["nov_poss"]
        else:
            replay_buffer["nov_intra"] = replay_buffer["itemsID"].apply(lambda values: np.zeros(len(values)))
...
class DataLoader(IterableDataset):
    def __init__(self, args, type =None, transforms: Optional[List[Transform]] = None):
        if type is None:
            replay_buffer = pd.read_pickle(args.data_path + 'train_test/train_session_df_replay_buffer.df')
        ...
        replay_buffer = _ensure_novelty_columns(replay_buffer)
        self.num_session_for_user = np.r_[0, replay_buffer.groupby("userID", sort=False)["sessionID"].nunique().cumsum().values]
    ...
    def __iter__(self):
        ...
        while not finished:
            ...
            user_change = np.arange(batch_size)[user_session_end - user_session_start <= 0]#session change是1，取出change的user位置
...
class TestDataLoader(IterableDataset):
    def __init__(self, args, type ='test', transforms: Optional[List[Transform]] = None):
        if type is None:
            ...
            replay_buffer = pd.read_pickle(args.data_path + 'train_test/test_session_df_replay_buffer.df')
        else:
            raise Exception('dataset has to be either None, val or test')
...
class DataLoaderClick(IterableDataset):
    def __init__(self, args, type =None, transforms: Optional[List[Transform]] = None):
        if type is None:
            ...
            replay_buffer = pd.read_pickle(args.data_path + 'train_test/test_df_replay_buffer.df')
        else:
            raise Exception('dataset has to be either None, val or test')
...
class DataLoaderUDT(IterableDataset):
    def __init__(self, args, type =None, transforms: Optional[List[Transform]] = None):
        if type is None:
            replay_buffer = _read_replay_buffer(args, "train", "train_test/train_session_df_replay_buffer_nov1_user.df")
...
class TestDataLoaderUDT(IterableDataset):
    def __init__(self, args, type ='test', transforms: Optional[List[Transform]] = None):
        if type is None:
            ...
            replay_buffer = _read_replay_buffer(args, "test", "train_test/test_session_df_replay_buffer_nov1_users_h.df")
        else:
            raise Exception('dataset has to be either None, val or test')
    ...
    def __iter__(self):
        ...
        while not finished:
            ...
            user_session_start += min_session_length #user_session_start增大经过的长度，
            user_change = np.arange(batch_size)[user_session_end - user_session_start <= 0]#session change是1，取出change的user位置
            #新user在一个batch里面的位置
            for idx in user_change:
                if max_user_iter + 1 >= len(self.users):
