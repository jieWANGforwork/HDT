...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, queries, keys, values=None, res_q=None):
        """
        ...
        if values is not None:
            V = self.linear_v(values)  # (N, T_k, C)
        else:
        ...
        if keys.shape[1] != 2:
            ...
            #mask的值是-63， 不mask的是原值

            # Activation
            matmul_output_sm = self.softmax(matmul_output_m2)  # (h*N, T_q, T_k)
        else:
            matmul_output_sm = self.softmax(matmul_output)
        ...
        if res_q is not None:
            output_res = output + res_q
        else:
            output_res = output + queries
        '''output_res = self.user_gru( queries.view(output.shape[0] * 20, -1),
                                    output.view(output.shape[0] * 20, -1)).view(output.shape[0], 20, -1)'''

        return output_res


class MultiHeadAttentionNores(nn.Module):
    def __init__(self, hidden_size, num_units, num_heads, dropout_rate):
        super().__init__()
        self.hidden_size = hidden_size
        self.num_heads = num_heads
    ...
    def forward(self, queries, keys, values=None):
        ...
        return output_res, output, queries
...
class MultiHeadAttentionCross(nn.Module):
    ...
    def forward(self, queries, keys):
        ...
        :return: A 3d tensor with shape of (N, T_q, C)
        1. 正常的在获得score后加上负无穷再softmax
        2. 第二种是在softmax后给mask的乘0
        """
        Q = self.linear_q(queries)  # (N, T_q, C)
        K = self.linear_k(keys)  # (N, T_k, C)
        V = self.linear_v(keys)  # (N, T_k, C)
        ...
        # Activation
        #matmul_output_sm = self.softmax(matmul_output_m2)  # (h*N, T_q, T_k)
        matmul_output_sm = self.softmax(matmul_output)  # (h*N, T_q, T_k)
