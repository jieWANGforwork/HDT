...
class Attention(nn.Module):
    ...
    def _attn(self, q, k, v, attention_mask=None, head_mask=None, output_attentions=False, intra=False):
        w = torch.matmul(q, k)
        if self.scale:
        ...
        if not self.is_cross_attention:
            ...
            #print(mask.shape, w.shape)
            w = torch.where(mask.bool(), w, self.masked_bias.to(w.dtype))

        if attention_mask is not None and intra:
    ...
    def forward(
        ...
        attn_outputs = self._attn(query, key, value, attention_mask, head_mask, output_attentions, intra=intra)
        a = attn_outputs[0]
